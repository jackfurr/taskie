...
(function () {
					...
					getSchemaVersion(connection, function(err, schemaVersion) {
						...
						getPatchFileArray(function(err, list) {
							...
							for (let i = 0; i < list.length; i++) {
								executePatch(connection, list, i, handle_executePatch);
							}
						});
	...
	function handle_executePatch(err, result, list, index, connection) {
		processedList.push(list[index]);
	...
	function executePatch(connection, list, index, callback) {
		console.log('Execute patch: ' + list[index]);
		let data = fs.readFileSync(list[index], 'utf8');
		let fileSchemaA = path.parse(list[index]);
		let newSchemaVer = parseInt(fileSchemaA.name);

		// execute the contents
		connection.query(data, '', function(err, result) {
			if (err) {
				return callback(err, null, list, index, connection);
			} else {
				...
				connection.query('UPDATE schema_version SET patch_level = ?', newSchemaVer, function(err, result) {
					return callback(err, result, list, index, connection);
				});
			}
		});
	}
